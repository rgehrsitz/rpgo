...
func (acf *AnnualCashFlow) CalculateTotalIncome() decimal.Decimal {
	return acf.PensionRobert.Add(acf.PensionDawn).
...
func (acf *AnnualCashFlow) CalculateTotalDeductions() decimal.Decimal {
	return acf.FederalTax.Add(acf.StateTax).Add(acf.LocalTax).
		Add(acf.FEHBPremium).Add(acf.MedicarePremium)
}

// CalculateNetIncome calculates the net income after all deductions
func (acf *AnnualCashFlow) CalculateNetIncome() decimal.Decimal {
	acf.TotalGrossIncome = acf.CalculateTotalIncome()
