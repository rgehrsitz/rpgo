...
func TestMortalityBasic(t *testing.T) {
	...
	if len(projection) < 6 { t.Fatalf("expected >=6 years, got %d", len(projection)) }

	// Year index for 2030 relative to base year 2025
	deathYearIdx := deathDate.Year() - ProjectionBaseYear
	if deathYearIdx < 0 || deathYearIdx >= len(projection) { t.Fatalf("death year index out of range: %d", deathYearIdx) }

	before := projection[deathYearIdx-1]
	after := projection[deathYearIdx]
	next := projection[deathYearIdx+1]

	if after.RobertDeceased != true { t.Fatalf("expected Robert deceased in death year") }
	if before.RobertDeceased { t.Fatalf("Robert should be alive year before death") }

	// Robert pension and SS should be zero after death year start
	if after.PensionRobert.GreaterThan(decimal.Zero) { t.Fatalf("pension should cease after death; got %s", after.PensionRobert) }
	if after.SSBenefitRobert.GreaterThan(decimal.Zero) { t.Fatalf("SS should cease after death; got %s", after.SSBenefitRobert) }

	// Survivor (Dawn) should have SS at least her own prior-year or Robert's whichever higher (simplified rule)
	if after.SSBenefitDawn.LessThan(before.SSBenefitDawn) { t.Fatalf("survivor SS should not decrease; before %s after %s", before.SSBenefitDawn, after.SSBenefitDawn) }

	// Filing status should switch the year AFTER death per next_year policy
	if after.FilingStatusSingle { t.Fatalf("filing status should remain MFJ in death year for next_year policy") }
	if !next.FilingStatusSingle { t.Fatalf("filing status should be single the year after death") }
}
