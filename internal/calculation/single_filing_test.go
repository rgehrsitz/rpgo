...
func TestSingleBracketDefaults(t *testing.T) {
	...
	if !calc.StandardDeductionSingle.Equal(decimal.NewFromInt(15000)) { t.Fatalf("expected default single standard deduction 15000 got %s", calc.StandardDeductionSingle) }
	if len(calc.Brackets) == 0 || len(calc.BracketsSingle) != len(calc.Brackets) { t.Fatalf("expected mirrored single brackets") }
}
...
func TestMortalityFilingAffectsSSTaxation(t *testing.T) {
	...
	if deathIdx <= 0 || deathIdx >= len(proj) { t.Fatalf("deathIdx out of range") }
	pre := proj[deathIdx-1]
	post := proj[deathIdx]
	// Ensure filing status changed
	if !post.FilingStatusSingle { t.Fatalf("expected single filing status post death") }
	...
	if pre.SSBenefitRobert.Add(pre.SSBenefitDawn).IsZero() || post.SSBenefitRobert.Add(post.SSBenefitDawn).IsZero() { t.Skip("SS not started yet in this simplified test") }
}
